...
impl<'i> CheckFnCtxt<'i, '_> {
    ...
    fn check_mir_stmt(&mut self, mctx: &MetaContext<'i>, stmt: &'i pairs::MirStmt<'i>) {
        match stmt.deref() {
            ...
            Choice9::_5(mir_loop) => self.check_mir_loop(mctx, mir_loop),
            Choice9::_6(mir_switchint) => self.check_mir_switch_int(mctx, mir_switchint),
            Choice9::_7(mir_copy_non_overlapping) => {
                self.check_mir_copy_non_overlapping(mctx, mir_copy_non_overlapping.get_matched().0);
            },
            Choice9::_8(mir_macro) => {
                let place = mir_macro.get_matched().0.MirPlace();
                self.check_mir_place(mctx, place);
            },
        }
    }
    ...
    fn check_mir_fn_operand(&mut self, mctx: &MetaContext<'i>, fn_operand: &'i pairs::MirFnOperand<'i>) {
        match fn_operand.deref() {
            Choice6::_0(copy_) => self.check_mir_place(mctx, copy_.get_matched().1.MirPlace()),
            Choice6::_1(move_) => self.check_mir_place(mctx, move_.get_matched().1.MirPlace()),
            Choice6::_2(ty_path) => self.check_type_path(mctx, ty_path),
            Choice6::_3(lang_item) => self.check_lang_item_with_args(mctx, lang_item),
            Choice6::_4(_op_ref) => { /* validated by resolver in Task 7 */ },
